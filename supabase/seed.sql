...
on conflict (slug) do update set title = excluded.title, description = excluded.description, duration_weeks = excluded.duration_weeks, published = excluded.published;
...
on conflict (course_id, week_number) do update set title = excluded.title;

insert into public.academy_lessons (week_id, title, slug, lesson_number, objectives, content, published)
select w.id, lesson.title, lesson.slug, lesson.lesson_number, lesson.objectives, lesson.content::jsonb, true
from public.academy_weeks w
join public.academy_courses c on c.id = w.course_id
cross join (values (
  1,
  'Python foundations: values and variables',
  'python-foundations-values-and-variables',
  array['Name values with variables', 'Recognize common Python data types'],
  ...
  'functions-reusable-thinking',
  array['Define a function', 'Use parameters and return values'],
  ...
  'pandas-reading-a-dataset',
  array['Describe rows and columns', 'Inspect missing or surprising values'],
...
)) as lesson(week_number, title, slug, objectives, content)
where c.slug = 'python-for-ai-machine-learning' and w.week_number = lesson.week_number
on conflict (week_id, slug) do update set title = excluded.title, objectives = excluded.objectives, content = excluded.content, published = excluded.published;

insert into public.academy_projects (course_id, title, description)
select c.id, 'My first AI/ML investigation', 'Choose a small question, explore data, train a model, and explain what you discovered.'
from public.academy_courses c
where c.slug = 'python-for-ai-machine-learning'
  and not exists (select 1 from public.academy_projects p where p.course_id = c.id);

insert into public.academy_project_milestones (project_id, milestone_number, title)
select p.id, milestones.milestone_number, milestones.title
...
where not exists (select 1 from public.academy_project_milestones m where m.project_id = p.id and m.milestone_number = milestones.milestone_number);
