...
function useSessionHeartbeat(isStudent, userId) {
  ...
  useEffect(() => {
    ...
    startAcademyLearningSession(routeRef.current)
      .then(({ data }) => {
        if (!cancelled) learningSession.current = data;
        else if (data?.id) sendHeartbeat(data.id, "hidden", false);
      })
      .catch(() => {});
    ...
    const ACTIVITY_EVENTS = ["pointerdown", "keydown", "scroll", "mousemove"];
    ...
    const heartbeat = window.setInterval(() => {
      ...
      if (
        ...
        sendHeartbeat(learningSession.current.id, "visible", true);
      }
    }, 30000);
    ...
    const handleVisibilityChange = () => {
      if (document.visibilityState === "visible" && learningSession.current) {
        lastActivity.current = Date.now();
      } else if (learningSession.current) {
        sendHeartbeat(learningSession.current.id, "hidden", false);
      }
    };
